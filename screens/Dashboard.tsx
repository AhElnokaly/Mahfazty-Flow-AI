...
const Dashboard: React.FC = () => {
  ...
  const groups = useMemo(
    () => state.groups.filter((g) => !g.isArchived),
    [state.groups],
  );
  const clients = useMemo(
    () => state.clients.filter((c) => !c.isArchived),
    [state.clients],
  );
  ...
  const transactions = useMemo(() => {
    return state.transactions.filter((t) => {
      ...
      const hasArchivedClient = t.clientIds
        ? t.clientIds.some(
            (cId) => state.clients.find((c) => c.id === cId)?.isArchived,
          )
    ...
    .filter((t) => {
      try {
        return (
          isIncomeLike(t) &&
          t.date &&
          new Date(t.date).toISOString().startsWith(lastMonthStr)
        );
      } catch {
        return false;
      }
  ...
  transactions.forEach((t) => {
    if (t.isDebt) {
      ...
      else if (t.debtAction?.toUpperCase() === "REPAY_BORROW")
        totalDebtsIOwe -= t.amount;
      else if (t.debtAction?.toUpperCase() === "LEND")
        totalDebtsOwedToMe += t.amount;
      else if (t.debtAction?.toUpperCase() === "REPAY_LEND")
        totalDebtsOwedToMe -= t.amount;
      else {
      ...
      t.items.forEach((item) => {
        if (item.isDebt) {
          if (isExpenseLike(t))
            totalDebtsOwedToMe += item.price * item.quantity;
          else if (isIncomeLike(t))
            totalDebtsIOwe += item.price * item.quantity;
        }
      });
    }
  });

  const partialPayments = transactions.filter(
    (t) => t.referenceTotal && t.referenceTotal > t.amount,
  );
  ...
  installments
    ...
    .forEach((i) => {
      const remaining =
        (i.totalAmount || i.monthlyAmount * i.installmentCount) -
        i.paidCount * i.monthlyAmount;
  ...
  const investmentTransactions = transactions.filter(
    (t) => t.type?.toUpperCase() === "INVESTMENT",
  );
  const investedCapital = investmentTransactions
    .filter(
      (t) =>
        t.investmentAction?.toUpperCase() !== "SELL" &&
        t.investmentAction?.toUpperCase() !== "RETURN",
    )
    .reduce((s, t) => s + t.amount, 0);
  ...
  const accountCards = useMemo(() => {
    ...
    return groups.map((group, index) => {
      const groupTransactions = transactions.filter((t) =>
        isTxRelatedToGroup(t, group.id, clients),
      );
        ...
        .reduce(
          (sum, t) =>
            ...
            clients
              ...
              .reduce((s, c) => s + getClientShare(t, c.id), 0),
          0,
        );
          ...
          value: transactions
            .filter(
              (t) =>
                ...
                (isUncat ? !t.categoryId : t.categoryId === cat.id),
            )
            .reduce((s, t) => s + t.amount, 0),
        };
      })
  ...
  const topExpenseCategory =
    expenseBreakdown.length > 0 ? expenseBreakdown[0] : null;
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const dailyNet = transactions
        .filter((t) => {
          try {
            return (
              t.date && new Date(t.date).toISOString().split("T")[0] === dayStr
            );
          } catch {
            return false;
          }
  ...
  const breakdown = useMemo(() => {
    const isAdjustment = (t: any) =>
      t.groupId === "system_adjustment" && t.clientId === "system_adjustment";
    ...
    const totalInstallmentsPaid = installments.reduce(
      (sum, i) => sum + i.paidCount * i.monthlyAmount,
      0,
    );
    const calculatedBalance =
      totalIncome - totalExpense - totalInstallmentsPaid;
    ...
    const totalInvestments = transactions
      ...
      .reduce((sum, t) => {
        if (
          t.investmentAction?.toUpperCase() === "BUY" ||
          t.investmentAction?.toUpperCase() === "FREE_STOCK"
        )
          return sum + t.amount;
        if (
          t.investmentAction?.toUpperCase() === "SELL" ||
    ...
    transactions.forEach((t) => {
      if (t.isDebt) {
        ...
        clientsForTx.forEach((cId) => {
          ...
          if (t.debtAction?.toUpperCase() === "BORROW")
            clientDebts[cId] += amountPerClient;
          else if (t.debtAction?.toUpperCase() === "REPAY_BORROW")
            clientDebts[cId] -= amountPerClient;
          else if (t.debtAction?.toUpperCase() === "LEND")
            clientDebts[cId] -= amountPerClient;
        ...
        t.items.forEach((item) => {
          if (item.isDebt && item.clientId) {
            ...
            if (isExpenseLike(t))
              clientDebts[item.clientId] -= item.price * item.quantity; // They owe me (LEND)
            else if (isIncomeLike(t))
              clientDebts[item.clientId] += item.price * item.quantity; // I owe them (BORROW)
          }
        });
      }
    });
  ...
  const isOverBudget =
    currentMonthIncome > 0 && currentMonthExpense > currentMonthIncome * 0.8;
  const budgetPercentage =
    currentMonthIncome > 0
      ? Math.round((currentMonthExpense / currentMonthIncome) * 100)
      : 0;
  // +++ نهاية الإضافة +++
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700 pb-20 px-2 max-w-5xl mx-auto">
      ...
      <section
        ...
        className={`bg-gradient-to-br from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-900 p-8 rounded-[2rem] text-white shadow-xl shadow-slate-900/10 flex flex-col items-center justify-center text-center relative overflow-hidden cursor-pointer hover:shadow-2xl transition-all duration-500 group ${balanceAnimation === "up" ? "scale-[1.02] ring-4 ring-emerald-500/50" : balanceAnimation === "down" ? "scale-[0.98] ring-4 ring-rose-500/50" : ""}`}
      >
        {/* Decorative Grid & Blur */}
        <div className="absolute inset-0 bg-black/10 backdrop-blur-sm pointer-events-none" />
        <div className="absolute -top-24 -left-24 w-48 h-48 bg-blue-500/20 blur-[60px] rounded-full pointer-events-none transition-all duration-700 group-hover:bg-blue-400/30"></div>
        <div className="absolute -bottom-24 -right-24 w-48 h-48 bg-purple-500/20 blur-[60px] rounded-full pointer-events-none transition-all duration-700 group-hover:bg-purple-400/30"></div>
        ...
        <div className="absolute bottom-[-20%] right-[-10%] opacity-10 pointer-events-none w-48 h-48">
        ...
        <div className="relative z-10 w-full flex flex-col items-center">
          <div className="flex items-center gap-2 mb-2 bg-white/10 px-4 py-1.5 rounded-full backdrop-blur-md border border-white/5">
            ...
            >
              <Info size={14} className="text-white opacity-80" />
            </button>
          </div>

          <h2
            className={`text-4xl md:text-5xl font-black text-white tracking-tighter drop-shadow-md transition-all duration-300 ${balanceAnimation === "up" ? "text-emerald-300 drop-shadow-[0_0_15px_rgba(52,211,153,0.5)]" : balanceAnimation === "down" ? "text-rose-300 drop-shadow-[0_0_15px_rgba(251,113,133,0.5)]" : ""} ${privacyClass}`}
          >
            {baseCurrency} {walletBalance.toLocaleString()}
          </h2>

          {isBalanceExpanded && (
            <div className="mt-8 w-full max-w-sm space-y-3 animate-in fade-in slide-in-from-top-4 duration-500 border-t border-white/10 pt-6">
              <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl">
                <span className="text-sm font-bold text-slate-300">
                  {language === "ar" ? "الكاش" : "Cash"}
                </span>
                <span
                  className={`text-sm font-black text-white ${privacyClass}`}
                >
                  {baseCurrency} {walletBalance.toLocaleString()}
                </span>
              </div>
              ...
              {breakdown.totalDebtsOwedByMe > 0 && (
                <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl">
                  ...
                  <span
                    className={`text-sm font-black text-rose-300 drop-shadow-[0_0_8px_rgba(251,113,133,0.3)] ${privacyClass}`}
                  >
                    -{baseCurrency}{" "}
                    {breakdown.totalDebtsOwedByMe.toLocaleString()}
                  </span>
                </div>
              )}
              ...
              {breakdown.totalDebtsOwedToMe > 0 && (
                <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl">
                  <span className="text-sm font-bold text-slate-300">
                    {language === "ar" ? "ديون مستحقة لك" : "Debts Owed To You"}
                  </span>
                  <span
                    className={`text-sm font-black text-emerald-300 drop-shadow-[0_0_8px_rgba(52,211,153,0.3)] ${privacyClass}`}
                  >
                    +{baseCurrency}{" "}
                    {breakdown.totalDebtsOwedToMe.toLocaleString()}
                  </span>
                </div>
              )}
              ...
              {breakdown.totalInvestments > 0 && (
                <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl">
                  ...
                  <span
                    className={`text-sm font-black text-indigo-300 drop-shadow-[0_0_8px_rgba(99,102,241,0.3)] ${privacyClass}`}
                  >
                    {baseCurrency} {breakdown.totalInvestments.toLocaleString()}
                  </span>
                </div>
              )}
            </div>
          ...
          {balanceDiff !== null && (
            <div
              className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-2xl font-black pointer-events-none animate-in slide-in-from-bottom-8 fade-in duration-700 slide-out-to-top-12 fade-out ${balanceDiff > 0 ? "text-emerald-400 drop-shadow-[0_0_10px_rgba(52,211,153,0.8)]" : "text-rose-400 drop-shadow-[0_0_10px_rgba(251,113,133,0.8)]"}`}
            >
              {balanceDiff > 0 ? "+" : ""}
            ...
            <div className="p-6 space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800/50">
                ...
                <span className="text-sm font-black text-emerald-600">
                  +{baseCurrency} {breakdown.totalIncome.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800/50">
                <span className="text-sm font-bold text-slate-500">
                ...
                <span className="text-sm font-black text-rose-600">
                  -{baseCurrency} {breakdown.totalExpense.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800/50">
                <span className="text-sm font-bold text-slate-500">
                  {language === "ar" ? "أقساط مدفوعة" : "Paid Installments"}
                </span>
                <span className="text-sm font-black text-rose-600">
                  -{baseCurrency}{" "}
                  {breakdown.totalInstallmentsPaid.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800/50">
                  ...
                  <button
                    onClick={() => {
                      const newBalance = prompt(
                        language === "ar"
                          ? "أدخل الرصيد الافتتاحي الجديد:"
                          : "Enter new starting balance:",
                        breakdown.adjustment.toString(),
                      );
                      if (
                        newBalance !== null &&
                ...
                <span
                  className={`text-sm font-black ${breakdown.adjustment >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                >
                  {breakdown.adjustment > 0 ? "+" : ""}
                  {baseCurrency} {breakdown.adjustment.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between items-center pt-4 mt-2">
                <span className="text-base font-black uppercase tracking-widest text-slate-900 dark:text-white">
                  {language === "ar" ? "الرصيد النهائي" : "Final Balance"}
                </span>
              ...
              {(() => {
                ...
                transactions.forEach((t) => {
                  if (t.isDebt) {
                    ...
                    t.items.forEach((item) => {
                      if (item.isDebt && item.clientId) {
                        if (!clientDebts[item.clientId])
                          clientDebts[item.clientId] = 0;
                        if (isExpenseLike(t))
                          clientDebts[item.clientId] -=
                ...
                partialPayments.forEach((t) => {
                  const remaining = (t.referenceTotal || 0) - t.amount;
                  if (t.type?.toUpperCase() === "EXPENSE")
                    totalDebtsIOwe += remaining;
                  else if (t.type?.toUpperCase() === "INCOME")
                    totalDebtsOwedToMe += remaining;
                });

                if (totalDebtsIOwe === 0 && totalDebtsOwedToMe === 0)
                  return null;
                  ...
                  <div className="pt-4 mt-2 border-t border-slate-100 dark:border-slate-800/50 space-y-2">
                    {totalDebtsIOwe > 0 && (
                      <div className="flex justify-between items-center py-1">
                        ...
                        <span className="text-sm font-black text-rose-600">
                          {baseCurrency} {totalDebtsIOwe.toLocaleString()}
                        </span>
                      </div>
                    )}
                    {totalDebtsOwedToMe > 0 && (
                      <div className="flex justify-between items-center py-1">
                        ...
                        <span className="text-sm font-black text-emerald-600">
                          {baseCurrency} {totalDebtsOwedToMe.toLocaleString()}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-4 mt-2 border-t border-slate-100 dark:border-slate-800/50 space-y-2">
                    <div className="flex justify-between items-center py-1">
                      ...
                      <span className="text-sm font-black text-purple-600">
                        {baseCurrency} {totalInvestments.toLocaleString()}
                      </span>
                    </div>
                  </div>
                );
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  <h3 className="text-sm font-bold uppercase tracking-wide">
                    {language === "ar" ? "فحص مالي ذكي" : "AI Financial Audit"}
                  </h3>
                  <div className="group/ai relative flex items-center">
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 p-2 bg-white text-slate-900 text-[10px] font-bold rounded-xl opacity-0 invisible group-hover/ai:opacity-100 group-hover/ai:visible transition-all z-50 text-center pointer-events-none shadow-xl">
                      {language === "ar"
                        ? "يقوم الذكاء الاصطناعي بتحليل مصاريفك وتقديم نصائح مخصصة لتقليل النفقات وزيادة المدخرات."
                        : "AI analyzes your spending and provides personalized tips to reduce expenses and increase savings."}
              ...
              <div
                className={`flex items-center gap-1.5 px-2 py-1 rounded-full border border-white/20 ${isPro ? "bg-amber-400 text-amber-950" : "bg-white/10 text-white/60"}`}
              >
                {isPro ? (
            ...
            <div className="flex items-center justify-between">
              <p className="text-xs font-bold text-blue-100/80 leading-relaxed max-w-[300px]">
                {isPro
                  ? language === "ar"
                    ? "تم تفعيل Gemini Pro لتحليل عميق لتوجهات السوق والأسعار الحقيقية."
                    : "Gemini Pro active: Deep analysis of market trends and real-time prices."
                  : language === "ar"
                    ? "احصل على تحليل فوري لنفقاتك وتوصيات لتحسين ادخارك."
                    : "Get instant analysis of your spending and tips to boost savings."}
              </p>
              <ArrowUpRight
                size={24}
      ...
      {activeTab === "overview" && (
        <>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Cashflow Stat */}
            <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-800/80 p-5 rounded-[1.5rem] border border-slate-100 dark:border-slate-700/50 shadow-sm flex flex-col justify-between group hover:shadow-lg hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                <p className="text-xs font-bold uppercase tracking-wide text-slate-500">
                  {language === "ar" ? "السيولة النقدية" : "Cashflow"}
                </p>
                <div className="w-8 h-8 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-500 rounded-xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-transform">
              ...
              <div className="space-y-2 relative z-10">
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`text-sm font-black text-emerald-600 dark:text-emerald-400 ${privacyClass}`}
                  >
                    +{baseCurrency} {currentMonthIncome.toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">
                    {language === "ar" ? "المنصرف" : "Expense"}
                  </span>
                  <span
                    className={`text-sm font-black text-rose-600 dark:text-rose-400 ${privacyClass}`}
                  >
                    -{baseCurrency} {currentMonthExpense.toLocaleString()}
                  </span>
                </div>
                <div className="pt-2 border-t border-slate-100 dark:border-slate-700/50 flex justify-between items-center">
                  ...
                  <span
                    className={`text-base font-black ${currentMonthIncome - currentMonthExpense >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"} ${privacyClass}`}
                  >
                    {baseCurrency}{" "}
                    {(
            ...
            <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-800/80 p-5 rounded-[1.5rem] border border-slate-100 dark:border-slate-700/50 shadow-sm flex flex-col justify-between group hover:shadow-lg hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                <p className="text-xs font-bold uppercase tracking-wide text-slate-500">
                  {language === "ar" ? "موقف الديون" : "Debt Position"}
                </p>
                <div className="w-8 h-8 bg-orange-50 dark:bg-orange-500/10 text-orange-500 rounded-xl flex items-center justify-center group-hover:scale-110 group-hover:-rotate-3 transition-transform">
              ...
              <div className="space-y-2 relative z-10">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">
                    {language === "ar" ? "ديون لك" : "Owed to You"}
                  </span>
                  <span
                    className={`text-sm font-black text-emerald-600 dark:text-emerald-400 ${privacyClass}`}
                  >
                ...
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">
                    {language === "ar" ? "ديون وأقساط عليك" : "You Owe"}
                  </span>
                  <span
                    className={`text-sm font-black text-rose-600 dark:text-rose-400 ${privacyClass}`}
                  >
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-700/50 flex justify-between items-center">
                  ...
                  <span
                    className={`text-base font-black ${totalDebtsOwedToMe - totalDebtsIOwe >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"} ${privacyClass}`}
                  >
                    {baseCurrency}{" "}
                    {(totalDebtsOwedToMe - totalDebtsIOwe).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-800/80 p-5 rounded-[1.5rem] border border-slate-100 dark:border-slate-700/50 shadow-sm flex flex-col justify-between group hover:shadow-lg hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                ...
                <div className="w-8 h-8 bg-blue-50 dark:bg-blue-500/10 text-blue-500 rounded-xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-transform">
              ...
              <div className="space-y-2 relative z-10">
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`text-sm font-black text-blue-600 dark:text-blue-400 ${privacyClass}`}
                  >
                    {baseCurrency} {investedCapital.toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">
                    {language === "ar" ? "عوائد ومبيعات" : "Returns & Sales"}
                  </span>
                  <span
                    className={`text-sm font-black text-emerald-600 dark:text-emerald-400 ${privacyClass}`}
                  >
                    {baseCurrency} {investmentReturns.toLocaleString()}
                  </span>
                </div>
                <div className="pt-2 border-t border-slate-100 dark:border-slate-700/50 flex justify-between items-center">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">
                  ...
                  <span
                    className={`text-base font-black ${investmentReturns - investedCapital >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"} ${privacyClass}`}
                  >
                    {baseCurrency}{" "}
                    {(investmentReturns - investedCapital).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
            ...
            {(() => {
              ...
              const amountDueThisMonth = creditCards.reduce(
                (sum, card) => sum + (card.balance > 0 ? card.balance : 0),
                0,
              );
              ...
              return (
                <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-800/80 p-5 rounded-[1.5rem] border border-slate-100 dark:border-slate-700/50 shadow-sm flex flex-col justify-between group hover:shadow-lg hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-4 relative z-10">
                    ...
                    <div className="w-8 h-8 bg-purple-50 dark:bg-purple-500/10 text-purple-500 rounded-xl flex items-center justify-center group-hover:scale-110 group-hover:-rotate-3 transition-transform">
                      <Wallet size={16} strokeWidth={3} />
                    </div>
                  </div>
                  <div className="space-y-2 relative z-10">
                    <div className="flex justify-between items-center">
                      ...
                      <span
                        className={`text-sm font-black text-emerald-600 dark:text-emerald-400 ${privacyClass}`}
                      >
                        {baseCurrency} {totalAvailable.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-bold text-slate-400 uppercase">
                      ...
                      <span
                        className={`text-sm font-black text-rose-600 dark:text-rose-400 ${privacyClass}`}
                      >
                        {baseCurrency} {amountDueThisMonth.toLocaleString()}
                      </span>
                    </div>
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-700/50 flex justify-between items-center">
                      <span className="text-[10px] font-bold text-slate-500 uppercase">
                        {language === "ar" ? "إجمالي المستحق" : "Total Due"}
                      </span>
                      <span
                        className={`text-base font-black text-rose-600 dark:text-rose-400 ${privacyClass}`}
                      >
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-1.5">
              <h3 className="text-xs font-bold uppercase tracking-wide text-slate-500">
                {language === "ar" ? "مجموعات الحسابات" : "Group Portfolios"}
              </h3>
              <div className="group relative flex items-center">
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 p-2 bg-slate-800 dark:bg-slate-700 text-white text-[10px] font-medium rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 text-center pointer-events-none shadow-xl">
                  {language === "ar"
                    ? "المجموعات تساعدك في تقسيم أموالك (مثل: حساب البنك، الكاش، المدخرات) وتحديد ميزانية لكل منها."
                    : "Portfolios help you divide your money (e.g., Bank, Cash, Savings) and set budgets for each."}
            ...
            <button
              ...
              className="text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-wide hover:underline flex items-center gap-1"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {accountCards.length > 0 ? (
              accountCards.map((account) => {
                ...
                const clientStats = groupClients.map((client) => {
                  ...
                  const clientTxs = transactions.filter((t) =>
                    isTxRelatedToClient(t, client.id),
                  );

                  const income = clientTxs
                    .filter((t) => isIncomeLike(t))
                    .reduce((s, t) => {
                      return s + getClientShare(t, client.id); // +++ أضيف بناءً على طلبك +++
                    }, 0);

                  const expense = clientTxs
                    .filter((t) => isExpenseLike(t))
                    .reduce((s, t) => {
                      return s + getClientShare(t, client.id); // +++ أضيف بناءً على طلبك +++
                    }, 0);
                ...
                return (
                  <div key={account.id} className="flex flex-col gap-2">
                    <div
                      onClick={() =>
                        setExpandedGroupId(isExpanded ? null : account.id)
                      }
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        ...
                        <h4 className="text-xs font-bold uppercase tracking-wide opacity-80 mb-1">
                          {account.name}
                        </h4>
                        <p
                          className={`text-xl font-black tracking-tighter ${privacyClass}`}
                        >
                          ...
                          <div className="mt-2 inline-block bg-white/20 backdrop-blur-md px-2 py-1 rounded-md">
                            <p className="text-[10px] font-bold uppercase tracking-wide">
                              {language === "ar"
                                ? "الرصيد المخصص:"
                                : "Allocated:"}{" "}
                              ${account.allocatedAmount.toLocaleString()}
                            </p>
                          </div>
                        )}
                        ...
                        {account.monthlyBudget > 0 && (
                          <div className="mt-3">
                            <div className="flex justify-between text-[10px] font-bold uppercase tracking-wide opacity-90 mb-1">
                              <span>
                                {Math.min(
                                  (account.expense / account.monthlyBudget) *
                                    100,
                                ).toFixed(0)}
                                % Used
                              </span>
                            ...
                            <div className="h-1 bg-black/20 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min((account.expense / account.monthlyBudget) * 100, 100)}%`,
                                }}
                              />
                            </div>
                          </div>
                          ...
                          <div className="space-y-3">
                            {clientStats.map((client) => {
                              ...
                              return (
                                <div
                                  key={client.id}
                                  className="border border-slate-100 dark:border-slate-700 rounded-2xl overflow-hidden"
                                >
                                  <div
                                    onClick={() =>
                                      setExpandedClientId(
                                        isClientExpanded ? null : client.id,
                                      )
                                    }
                                    ...
                                    <div className="flex items-center gap-4 text-xs font-bold">
                                      <div className="flex flex-col items-end gap-1">
                                        ...
                                        <span className="text-rose-500 flex items-center gap-1">
                                          ...
                                          {client.expense.toLocaleString()}
                                        </span>
                                      </div>
                                      <ChevronRight
                                        size={16}
                                        className={`text-slate-400 transition-transform ${isClientExpanded ? "rotate-90" : ""}`}
                                      />
                                    </div>
                                  </div>
              ...
              <div
                className="col-span-full bg-slate-50 dark:bg-slate-800/50 border border-dashed border-slate-200 dark:border-slate-700 rounded-3xl p-8 flex flex-col items-center justify-center text-center group hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
                onClick={() => navigate("/installments")}
              >
                ...
                <p className="text-xs text-slate-500 font-medium max-w-xs mb-6">
                  {language === "ar"
                    ? "قم بإنشاء مجموعة حسابات (مثل: المحفظة، البنك، مدخرات) لتنظيم أموالك."
                    : "Create a portfolio (e.g., Wallet, Bank, Savings) to organize your money."}
                </p>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[10px] font-black uppercase tracking-[4px] text-slate-400">
                  {language === "ar" ? "أحدث المعاملات" : "Recent Transactions"}
                </h3>
                <button
                  onClick={() => navigate("/history")}
                  className="text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase tracking-widest hover:underline flex items-center gap-1"
                >
                ...
                {recentTransactions.length > 0 ? (
                  recentTransactions.map((t) => {
                    ...
                    const primaryClient = clients.find(
                      (c) =>
                        c.id ===
                        (t.clientIds && t.clientIds.length > 0
                          ? t.clientIds[0]
                          : t.clientId),
                    );
                    const clientNameDisplay =
                      t.clientIds && t.clientIds.length > 1
                    ...
                    return (
                      <div
                        key={t.id}
                        className={`flex items-center justify-between p-3 rounded-2xl bg-slate-50 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors cursor-pointer relative overflow-hidden ${group?.color ? group.color.replace("bg-", "border-l-4 border-l-") : ""}`}
                        onClick={() => navigate("/history")}
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${isIncome ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30" : isCredit ? "bg-purple-100 text-purple-600 dark:bg-purple-900/30" : "bg-rose-100 text-rose-600 dark:bg-rose-900/30"}`}
                          >
                            {group?.icon || "💰"}
                          </div>
                          <div>
                            <p className="text-xs font-bold text-slate-900 dark:text-white">
                              {clientNameDisplay || t.note || "Transaction"}
                            </p>
                        ...
                        <div className="flex flex-col items-end">
                          <p
                            className={`text-sm font-black ${isIncome ? "text-emerald-600 dark:text-emerald-400" : isCredit ? "text-purple-600 dark:text-purple-400" : "text-rose-600 dark:text-rose-400"} ${privacyClass}`}
                          >
                            {isIncome ? "+" : "-"}
                          ...
                          {t.referenceTotal && (
                            <span className="text-[10px] text-slate-400 font-bold mt-0.5">
                              {language === "ar" ? "من أصل" : "out of"}{" "}
                              {baseCurrency} {t.referenceTotal.toLocaleString()}
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <p className="text-[10px] text-slate-500 font-medium mb-4">
                      {language === "ar"
                        ? "أضف أول دخل أو مصروف لك."
                        : "Add your first income or expense."}
                    </p>
                    <button
                      onClick={() => navigate("/add")}
                      className="text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-4 py-2 rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors"
                    >
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[10px] font-black uppercase tracking-[4px] text-slate-400">
                  {language === "ar" ? "الالتزامات القادمة" : "Upcoming Bills"}
                </h3>
                <button
                  onClick={() => navigate("/installments")}
                  className="text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase tracking-widest hover:underline flex items-center gap-1"
              ...
              <div className="space-y-3">
                {upcomingInstallments.length > 0 ? (
                  upcomingInstallments.map((inst) => {
                    const progress =
                      (inst.paidCount / inst.installmentCount) * 100;
                    return (
                      <div
                        key={inst.id}
                        className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800"
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span
                            className={`text-xs font-black text-slate-900 dark:text-white ${privacyClass}`}
                          >
                            ${(inst.monthlyAmount || 0).toLocaleString()}/mo
                          </span>
                        </div>
                        <div className="flex items-center justify-between text-[9px] font-bold text-slate-400 mb-1 uppercase tracking-widest">
                          <span>
                            {inst.paidCount || 0} of{" "}
                            {inst.installmentCount || 1} Paid
                          </span>
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <p className="text-[10px] text-slate-500 font-medium mb-4">
                      {language === "ar"
                        ? "أضف أقساطك أو ديونك لتتبعها هنا."
                        : "Add your installments or debts to track them here."}
                    </p>
                    <button
                      onClick={() => navigate("/installments")}
                      className="text-[10px] font-black uppercase tracking-widest text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-4 py-2 rounded-xl hover:bg-amber-100 dark:hover:bg-amber-900/40 transition-colors"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {goals.slice(0, 3).map((goal) => {
                ...
                return (
                  <div
                    key={goal.id}
                    className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors"
                    onClick={() => navigate("/goals")}
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl ${goal.color} bg-opacity-10 dark:bg-opacity-20`}
                      >
                        {goal.icon}
                      </div>
                      <div>
                        ...
                        <p
                          className={`text-[10px] font-bold text-slate-500 ${privacyClass}`}
                        >
                          {baseCurrency}{" "}
                          {(goal.currentAmount || 0).toLocaleString()} /{" "}
                          {(goal.targetAmount || 1).toLocaleString()}
                        </p>
                      </div>
                    </div>
                    <div className="h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
      ...
      {activeTab === "insights" && (
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
            <h3 className="text-[10px] font-black uppercase tracking-[4px] text-slate-400 mb-4">
              {language === "ar" ? "توزيع المصاريف" : "Spending Mix"}
            </h3>
            {topExpenseCategory ? (
              <div className="flex items-center gap-8">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black uppercase text-rose-500 tracking-widest mb-3">
                    {language === "ar" ? "أعلى فئة صرف" : "Top Category"}
                  </p>
                  <button
                    onClick={() => navigate("/analytics")}
                    className="text-[10px] font-black uppercase tracking-widest text-blue-600 flex items-center gap-1 hover:underline"
                  >
                    {language === "ar" ? "التفاصيل" : "Analytics"}{" "}
                    <ChevronRight
                      size={12}
                      className={language === "ar" ? "rotate-180" : ""}
                    />
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
            <h3 className="text-[10px] font-black uppercase tracking-[4px] text-slate-400 mb-4">
              {language === "ar" ? "التدفق المالي" : "Financial Velocity"}
            </h3>
            ...
            <p className="text-[9px] font-bold text-slate-400 text-center mt-4 uppercase tracking-widest">
              {language === "ar"
                ? "محاكاة آخر 30 يوم"
